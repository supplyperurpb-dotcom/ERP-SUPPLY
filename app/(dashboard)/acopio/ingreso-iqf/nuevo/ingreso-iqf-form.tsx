...
import { crearIngresoIQFAction, actualizarIngresoIQFAction } from "@/lib/actions/ingreso-iqf-actions";
import { CAPACIDAD_MAXIMA_BANDEJAS_POR_PALLET as CAPACIDAD_MAXIMA } from "@/lib/constants/pallet";
import { VARIEDADES_IQF } from "@/lib/constants/iqf";
...
export function IngresoIQFForm({
  proveedores,
  tiposBandeja,
  tiposPallet,
  palletsAbiertos,
  ...
  /** Presente solo cuando el formulario edita un ingreso IQF ya existente. */
  edicion?: {
    ingresoId: string;
    valoresIniciales: IngresoIQFInput;
  ...
  const form = useForm<IngresoIQFInput>({
    resolver: zodResolver(ingresoIQFSchema),
    defaultValues: edicion?.valoresIniciales ?? {
      proveedorId: "",
      // El <input type="date"> trabaja con un string "YYYY-MM-DD" en la fecha
      // LOCAL del navegador; react-hook-form lo deja pasar tal cual y zod lo
      // convierte a Date recién al validar.
  ...
  async function onSubmit(data: IngresoIQFInput) {
    ...
    if (edicion) {
      toast.success("Ingreso IQF actualizado");
    ...
    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        ...
        <CardContent className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Debe coincidir con una fecha en la que Ingreso de Materia Prima registró la misma variedad.
            </p>
          </div>
        ...
        <CardHeader className="flex flex-row items-center justify-between space-y-0">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Descarte de planta por variedad, asignado a un pallet físico propio de IQF (máximo{" "}
              {CAPACIDAD_MAXIMA} bandejas por pallet).
            </p>
          </div>
              ...
              <div key={field.id} className="rounded-lg border p-4">
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <Input value="Descarte Planta" disabled readOnly />
                  </div>
                </div>
      ...
      <Dialog open={lineaDialogoAbierto !== null} onOpenChange={(open) => !open && setLineaDialogoAbierto(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Solo se muestran pallets IQF abiertos con espacio disponible.</DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div>
              <p className="mb-1 text-xs font-semibold uppercase text-muted-foreground">Pallets IQF abiertos</p>
              {opcionesDialogo.abiertos.length === 0 ? (
                <p className="text-sm text-muted-foreground">No hay pallets IQF abiertos con espacio disponible.</p>
              ) : (
                <div className="space-y-1">
      ...
      <div className="flex justify-end gap-2">
        ...
        <Button type="submit" disabled={form.formState.isSubmitting}>
          {form.formState.isSubmitting ? "Guardando..." : edicion ? "Guardar cambios" : "Registrar ingreso IQF"}
        </Button>
      </div>
    </form>
  );
